-- Gate 5: activation is email-only. Phone/SMS is not an activation dependency.
-- Users under 14 are rejected. Users 14–17 fail closed until a separately
-- verified, legally approved guardian workflow is implemented.
...
begin
  ...
  if p_guardian_consent_version is not null
    or p_guardian_consent_recorded_at is not null then
    raise exception using errcode = '22023', message = 'Guardian approval payload is not supported';
  end if;
  ...
  if v_age is null
    or v_age not between 18 and 100
